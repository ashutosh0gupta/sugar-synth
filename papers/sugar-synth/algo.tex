...
\choosefinal{extended version~\cite{arxiv-this}.}{ Appendix~\ref{sec:variations}.}
...
The dashed area are the absent nodes. Among the matching nodes, we have two cases.
...
Finally, the fourth situation is the rest of the node positions
...
\begin{theorem}
  If \textsc{SugarSynth}($\mu$, $d$, $n$) returns a set of rules $R$, then if $R$ produces a molecule that has depth less than $h$, the it is in $\mu$ (soundness).
  If it fails to synthesize rules, there are no rule set from the budget of depth $d$ and number $n$ (completeness).
\end{theorem}

The soundness is true due to the construction of constraints.
Our algorithm always terminates. At each iteration, at least one solution, i.e,
